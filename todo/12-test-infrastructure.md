...
- [x] **[P12-01]** Configure **GitHub Actions** CI pipeline: Dockerized backend tests (`pytest` + PostGIS) and frontend tests (`vitest` + Playwright) run on every pull request. *(Can begin after Phase 1; blocks nothing else in this phase)* — backend CI done; frontend job deferred to [P2-05]/[P2-06] when the frontend exists
