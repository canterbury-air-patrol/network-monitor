...
- [ ] **[P4-01]** Add `djangorestframework-simplejwt` to `requirements.txt`. Configure DRF to use JWT authentication with session fallback for the Django Admin interface. *(Blocks: [P4-02])*
...
- [ ] **[P4-05]** Implement `Device` model with API key field using the mechanism chosen in [R-03]. Keys are non-recoverable after creation; implement secure generation and storage. Generate and apply migration. *(Requires: [R-03]; Blocks: [P4-06], [P4-07])*
...
- [ ] **[P4-09]** Enable `IsAuthenticated` on all remaining read endpoints. Audit every URL pattern to confirm no unauthenticated path to data remains. This task must be marked complete before Phase 5 begins. *(Requires: [P4-02], [P4-06]; Blocks: Phase 5)*
